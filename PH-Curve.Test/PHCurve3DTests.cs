...
namespace PHCurveLibrary.Tests
{
    ...
    public sealed class PHCurve3DTests
    {
        ...
        private static void AssertFrenetQuantities(
            PHCurve3D curve,
            Func<float, Vector3> analyticDeriv,
            Func<float, Vector3> analyticSecond)
        {
            float[] ts = new[] { 0f, 0.25f, 0.5f, 0.75f, 1f };
            foreach (float t in ts)
            {
                Vector3 expectedDeriv = analyticDeriv(t);
                Vector3 expectedSecond = analyticSecond(t);
                float expectedSpeed = expectedDeriv.Length();
                Vector3 expectedTangent = Vector3.Normalize(expectedDeriv);
                Vector3 expectedNormal = Vector3.Normalize(
                    (expectedSecond * expectedSpeed
                    - expectedDeriv * Vector3.Dot(expectedDeriv, expectedSecond) / expectedSpeed)
                    / (expectedSpeed * expectedSpeed));
                Vector3 expectedBitan = Vector3.Cross(expectedTangent, expectedNormal);
                float expectedCurv = Vector3.Cross(expectedDeriv, expectedSecond).Length()
                    / (expectedSpeed * expectedSpeed * expectedSpeed);
                ...
                System.Console.WriteLine($"t={t}: T={tangent}, N={normal}, B={bitan}, \u03ba={curvature}");
                AssertVector(expectedTangent, tangent, 1e-6f, $"Tangent at t={t}");
                AssertVector(expectedNormal, normal, 1e-6f, $"Normal at t={t}");
                AssertVector(expectedBitan, bitan, 1e-6f, $"BiTangent at t={t}");
                Assert.AreEqual(expectedCurv, curvature, 1e-6f, $"Curvature at t={t}");
            }
        }

        /// <summary>
        /// Verify that curvature, normal, tangent and bi-tangent are computed
        /// according to the Frenet–Serret formulas for both test curves at
        /// multiple parameter values.
        /// </summary>
        [TestMethod]
        public void FrenetFrameQuantities_AreCorrect()
        {
            PHCurve3D parabola = CreateParabola();
            AssertFrenetQuantities(
                parabola,
                t => new Vector3(1f, t, 0f),
                _ => new Vector3(0f, 1f, 0f));

            PHCurve3D higher = CreateHigherOrderCurve();
            AssertFrenetQuantities(
                higher,
                t => new Vector3(1f + MathF.Pow(t, 3f), t + MathF.Pow(t, 4f), t * t),
                t => new Vector3(3f * t * t, 1f + 4f * MathF.Pow(t, 3f), 2f * t));
        }

        /// <summary>
        /// Numerically integrate the speed using Simpson's rule. This helper is
        /// used for validating the analytic arc-length formula.
        /// </summary>
        private static float NumericalArcLength(PHCurve3D c, float t, int steps = 200)
        {
            float h = t / steps;
            float sum = c.Speed(0f) + c.Speed(t);
        ...
        /// <summary>
        /// Verify the exact arc-length for a simple planar PH curve with
        /// r'(t) = (1 + 2t, 2t + 2t², 0). The speed is 1 + 2t + 2t² whose
        /// integral has the closed form s(t) = t + t² + 2/3 t³.
        /// </summary>
        [TestMethod]
        public void ArcLength_PlanarPH_IsExact()
        {
            Vector3 a = new(1f, 0f, 0f);
            ...
            System.Console.WriteLine($"ArcLength_PlanarPH_IsExact: analytic {length}, numeric {numeric}");
            Assert.AreEqual(expected, length, 1e-5f);
            Assert.AreEqual(expected, numeric, 1e-3f);
        }

        /// <summary>
        /// The parabola is not a PH curve; nevertheless ArcLength should agree
        /// with the analytic integral ∫₀ᵗ sqrt(1 + u²) du.
        /// </summary>
        [TestMethod]
        public void ArcLength_NonPHCurve_FallsBackToNumerical()
        {
            PHCurve3D parabola = CreateParabola();
            float expected = 0.5f * (1f * MathF.Sqrt(2f) + MathF.Asinh(1f));
            float length = parabola.ArcLength(1f);

            System.Console.WriteLine($"ArcLength_NonPHCurve_FallsBackToNumerical: {length}");
            Assert.AreEqual(expected, length, 1e-3f);
        }

        /// <summary>
        /// A PH curve generated by <see cref="PHCurveFactory.CreateQuintic"/>
        /// must yield the same arc-length as numerical integration.
        /// </summary>
        [TestMethod]
        public void ArcLength_FactoryCurve_AgreesWithNumerical()
        {
            ...
            System.Console.WriteLine($"ArcLength_FactoryCurve_AgreesWithNumerical: analytic {analytic}, numeric {numeric}");
            Assert.AreEqual(numeric, analytic, 1e-4f);
        }

        /// <summary>
        /// The higher-order example defined by <see cref="CreateHigherOrderCurve"/>
        /// does not satisfy the PH condition. The arc-length must therefore
        /// fall back to Simpson integration and agree with the numerical result.
        /// </summary>
        [TestMethod]
        public void ArcLength_HigherOrderCurve_FallsBackToNumerical()
        {
            PHCurve3D curve = CreateHigherOrderCurve();
